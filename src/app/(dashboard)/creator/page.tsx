...
/** Traz o texto falado de um vídeo do YouTube para usar como base do roteiro */
function YoutubeComoBase({ aoTexto }: { aoTexto: (t: string) => void }) {
  const [url, setUrl] = useState('')
  const [carregando, setCarregando] = useState(false)
  const [erro, setErro] = useState('')
  const [res, setRes] = useState<{ titulo: string; canal: string; texto: string; markdown: string; palavras: number; tokens_aprox: number } | null>(null)
  ...
  async function buscar() {
    ...
    try {
      ...
      setRes(d)
      // o tema do roteiro vira "baseado neste vídeo" + o começo do que é falado
      aoTexto(`Baseado no vídeo "${d.titulo}"${d.canal ? ` (${d.canal})` : ''}: ${String(d.texto).slice(0, 1500)}`)
    } catch (e: any) {
      setErro(e.message)
    } finally {
      setCarregando(false)
  ...
  return (
    <div className="rounded-2xl bg-[#121216] border border-white/[0.08] p-3 space-y-2">
      <span className="text-[11px] font-semibold text-zinc-400 block">Ou use um vídeo do YouTube como base</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="space-y-2">
            ...
            />
            <YoutubeComoBase aoTexto={(t) => setTopic(t)} />
          </div>
