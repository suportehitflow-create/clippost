      ...
      {([
        ...
        <button
          ...
          className={`py-1.5 px-3 text-xs font-semibold rounded-lg transition-all cursor-pointer flex items-center gap-1.5 whitespace-nowrap ${
            aba === t.id ? 'bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-600 text-white shadow-sm' : 'text-zinc-400 hover:text-white'
          }`}
        >
    ...
    <div className="flex flex-col h-[100dvh] -mb-24 min-h-[620px] bg-[#0a0a0c]">
      {aba === 'editor' && !editorDisponivel ? (
        ...
        <div className="flex-1 min-h-0">
          <EditorMassa titulo="Edição em Massa" abasModo={abasModo} />
        </div>
      ) : (
        <>
