...
SYSTEM = (
    ...
    "- shadcn/ui components do NOT exist. Use plain Tailwind CSS only. No @/components/ui/* imports.\n"
    "- Supabase auth pattern (ONLY this pattern, no custom hooks):\n"
    "  import { createClient } from '@/lib/supabase/client'\n"
    "  const supabase = createClient()\n"
    "  const { data: { user } } = await supabase.auth.getUser()\n"
    "  Do NOT invent useSupabase(), useUser(), or any other custom hooks.\n"
    "- Backend API: const API = process.env.NEXT_PUBLIC_API_URL || 'https://clippost-backend.fly.dev'\n"
    "- Supabase client import in backend: from supabase import create_client (NOT from @supabase/ssr)\n"
